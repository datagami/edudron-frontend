...
//inter route API call
router.get('/getexample', async (req, res) => {
    try {
        // const { data: response } = await axios.get('http://localhost:8000/api/axios') //use data destructuring to get data from the promise object
        // res.send(response)
        let NotificatoinMsg = {
            title:"title test" , body:"body test"
        }
        firebaseToken = ['cH5MkWRhi-mDzaBqp2HI5L:APA91bGLFSDhLwf-vV38-AnrYzffDyFlHSId1K9R8NJ-yPsVRRtXaGk6URIxrv3gZhbDRbbjkadAs94OEOGl6Vtp-bTktJEZy9gbVGRvBYK1q3qrkvT7rN64aYA8yj3rd-xUTlL3shl9']
        const response = sendFireBaseNOtificationFCM(firebaseToken , NotificatoinMsg )
        return res.send(response)
    }
    // axios.post(api_url, dataString)
    catch (error) {
        console.log(error);
    }
})
...
router.post('/admin/send-notification' , jwt_auth , async(req,res)=>{
    const {sendto , title , body , image , soundtype , other} = req.body
    const NotificatoinMsg = {
        title , body , image , soundtype , other
    }
    ...
    else if(sendto == 'allAstrologer'){
        const newNotification = new Notification(req.body)
        await newNotification.save()

        console.log("Inside Allastrologer")
        const userDetail = await Users.updateMany({user_type:"astrologer"} ,{
            $push:{
                notifications:newNotification._id
            }
        } );
    ...
    else if(sendto == 'allMember'){
        const newNotification = new Notification(req.body)
        await newNotification.save()

        console.log("Inside allMember")
        const userDetail = await Users.updateMany({user_type:"member"} ,{
            $push:{
                notifications:newNotification._id
            }
        } );
...
router.get('/admin/list-notification/:type/:typedes' , jwt_auth , async (req ,res)=>{
...
//for users
router.get('/list-notification/:type/:typedes' , jwt_auth , async (req ,res)=>{
    const paramType  = req.params.type
    const paramTypeDes = req.params.typedes
       let notification
       var data = [];
    if(paramType=="trans_type"){
       ...
       const total_documents = await Notification.find({sendto:paramTypeDes}).countDocuments();
        ...
        const query = Notification.find({ $or: [
            { "sendto": "all" },
            { "sendto": "all_member" },
            { "sendto": req.user.ownerId },
            { "sendto_Id": { $in : [req.user.ownerId]  }}
